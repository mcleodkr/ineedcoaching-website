...
export async function clearTokensByZoomUserId(zoomUserId) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/coach_profiles?zoom_oauth_user_id=eq.${encodeURIComponent(zoomUserId)}`,
    {
      method: 'PATCH',
      headers: sbHeaders({
  ...
  if (!res.ok) throw new Error(`token clear by zoom user failed: ${res.status}`);
