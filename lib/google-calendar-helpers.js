...
// timezone + calendar connection state stay on coach_profiles. Fetch both and
// merge into the single shape the rest of this module expects. Service-role
// only — never expose this output to the client.
async function fetchCoachTokens(coachId) {
  const id = encodeURIComponent(coachId);
  const [profileRes, tokenRes] = await Promise.all([
    fetch(
      `${SUPABASE_URL}/rest/v1/coach_profiles?id=eq.${id}&select=id,timezone,google_calendar_enabled,google_calendar_id&limit=1`,
      { headers: sbHeaders() }
    ),
    fetch(
      `${SUPABASE_URL}/rest/v1/coach_oauth_tokens?coach_id=eq.${id}&select=google_access_token,google_refresh_token,google_token_expires_at&limit=1`,
      { headers: sbHeaders() }
    ),
  ]);
...
export async function saveCoachTokens(coachId, { accessToken, refreshToken, expiresIn }) {
  ...
  // makes that reliable, but Google may still omit it on re-link. Omitting it from
  // the merge payload leaves any existing stored refresh token untouched.
  if (refreshToken) tokenBody.google_refresh_token = refreshToken;
  ...
  // Connection-state flag stays on coach_profiles (read by the dashboard).
  const profileRes = await fetch(
    `${SUPABASE_URL}/rest/v1/coach_profiles?id=eq.${encodeURIComponent(coachId)}`,
    {
      method: 'PATCH',
      headers: sbHeaders({ 'Content-Type': 'application/json', Prefer: 'return=minimal' }),
      body: JSON.stringify({ google_calendar_enabled: true }),
    }
  );
...
export async function clearCoachTokens(coachId) {
  ...
  const profileRes = await fetch(
    `${SUPABASE_URL}/rest/v1/coach_profiles?id=eq.${id}`,
    {
      method: 'PATCH',
      headers: sbHeaders({ 'Content-Type': 'application/json', Prefer: 'return=minimal' }),
      body: JSON.stringify({ google_calendar_enabled: false }),
    }
  );
