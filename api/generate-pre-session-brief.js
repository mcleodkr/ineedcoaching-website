...
questions_to_explore — generate 5-8 strategic questions the coach can use during the session. These must be open-ended and explorative (not yes/no), connect directly to patterns_noticed and strategic_context when available, help close the gap between insight and embodied change, surface what has not been named yet, and check whether commitments moved from concept to felt experience. Each string starts with What / How / When / Where (rarely Why), references specific moments, body sensations, or real between-session experiences drawn from priorAnalyses, and avoids hypotheticals — ground every question in what actually happened. STRONG example: "What actually happened the last time you felt someone trying to tell you what to do — and what did you do with the pause?" WEAK examples to avoid: "How are you feeling about the work?" (too vague); "Did you practice the pause?" (yes/no, behavioral checkbox); "Why do you think you react that way?" (cognitive, not experiential). Populate this field even when coach_dna_alert or strategic_context are null — questions can be derived from patterns_noticed and priorAnalyses alone. Set to null ONLY if priorAnalyses is empty.
...
Return ONLY valid JSON with these exact keys:
{
  ...
  "questions_to_explore": ["array of 5-8 strings; each is an open-ended question starting with What/How/When/Where (rarely Why), grounded in specific between-session moments or body sensations from priorAnalyses, avoiding yes/no or cognitive framings. Populate even if coach_dna_alert and/or strategic_context are null. Set to null only when priorAnalyses is empty."],
