...
const PLAN_GUARDRAILS = `You are Coach Clarity, generating a longitudinal intervention plan for a coach working with one client. Read everything you are given. Output strictly valid JSON matching the schema below. Hard guardrails: every output item with a source_sessions field must populate it with real booking_ids; friction points carry forward verbatim into external_conditions; medication/substance/major-disruption conditions downgrade non-compliance interpretations; working_hypotheses use hedge language only with status testing|strengthening|weakening; modalities require source_evidence; strategic_frames whose primary_tool is set must define fallback_paths; tactical specificity is specific|contingent|directional inverse to session distance; commitments enumerate from prior sessions with status set by subsequent evidence; weave approach_lab_runs into modality_sequence and coach_dna into risk_watchouts/coach_commitment without prompting. Coaching tone only. Return ONLY raw JSON.`;
...
function validatePlan(planRaw, validBookingIds, activeGoalIds) {
  ...
  return {
    ...
    modality_sequence: (Array.isArray(planRaw.modality_sequence) ? planRaw.modality_sequence : []).filter(m => m && Array.isArray(m.source_evidence) && m.source_evidence.length > 0).map(m => ({ stage: [1,2,3].includes(m.stage) ? m.stage : 1, modality: m.modality || '', goal: m.goal || '', techniques: Array.isArray(m.techniques) ? m.techniques : [], source_evidence: m.source_evidence.filter(se => se && okBookings.has(String(se.session_id))).map(se => ({ session_id: se.session_id, quote: se.quote || '' })) })).filter(m => m.source_evidence.length > 0),
