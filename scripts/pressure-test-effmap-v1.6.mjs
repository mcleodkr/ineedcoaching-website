// Pressure test for the Effectiveness Map synthesis prompt v1.6.
// Run: node scripts/pressure-test-effmap-v1.6.mjs   (needs ANTHROPIC_API_KEY in env or .env.local)
//
// Three cases against the live model, then automated checks per the prompt's
// FINAL VERIFICATION list:
//   strong — rich answers; expects full Map, both how_this_shows_up moves possible
//   thin   — one-line answers; expects short output + bank release question
//   crisis — crisis language; expects the content-free crisis object ONLY
//
// Checks: JSON validity, required v1.6 structure, second-person voice, banned
// machinery vocabulary in explorer-facing text, coach synthesis present with
// framework vocabulary, evidence-strength behavior, crisis short-circuit.

import { readFileSync } from 'fs';
import { createRequire } from 'module';

const require = createRequire(import.meta.url);
const SYNTH = require('../api/prompts/effectiveness-map-synthesis-v1.6.json');
...
const CASES = {
  strong: {
    ...
    answers: {
      ...
      social_1: 'My partner, my sister, and two colleagues from my certification cohort know. One colleague sends me referrals already.',
...
// Banned machinery vocabulary — word-boundary, case-insensitive. "draining (as a
// label)" et al: status words are allowed on the status_label chips, banned in prose.
const BANNED = ['system', 'capacity', 'recruited', 'resourcing', 'draining', 'dominating',
  'unreleased', 'pacing function', 'bandwidth', 'activated', 'deployed', 'operating', 'functioning'];
// 'input' and 'domain' get their own patterns to avoid false-positive substrings.
const BANNED_RE = BANNED.map((w) => new RegExp(`\\b${w.replace(' ', '\\s+')}\\b`, 'i'))
  .concat([/\binputs?\b/i, /\bdomains?\b/i]);

function explorerText(map) {
...
function runChecks(label, map) {
  ...
  const text = explorerText(map);
  const words = text.split(/\s+/).filter(Boolean).length;

  fails += check('crisis_flag false', map.crisis_flag === false);
  fails += check('prompt_version 1.6', map.metadata && map.metadata.prompt_version === '1.6');
  ...
  fails += check('system_picture present', typeof ef.system_picture === 'string' && ef.system_picture.length > 50);
  ...
  const banned = BANNED_RE.map((re) => text.match(re)).filter(Boolean).map((m) => m[0]);
  fails += check('no banned vocabulary in explorer text', banned.length === 0, banned.length ? `found: ${[...new Set(banned)].join(', ')}` : '');
  fails += check('second person ("you/your" used)', /\byou\b/i.test(text) && /\byour\b/i.test(text));
  fails += check('never "the explorer" in explorer text', !/the explorer/i.test(text));
  fails += check('coach_synthesis present', typeof cf.coach_synthesis === 'string' && cf.coach_synthesis.length > 100);
  fails += check('coach_synthesis uses framework vocab', /(Resourcing|Draining|Dominating|Unreleased)/.test(cf.coach_synthesis));
  ...
  if (label === 'strong') {
    ...
    fails += check('explorer-facing 500-1300 words', words >= 500 && words <= 1300, `${words} words`);
  }
  if (label === 'thin') {
    fails += check('evidence thin', strength === 'thin', `strength=${strength}`);
    // The prompt sets no numeric thin bound — only "shorter is correct, never
    // pad". Measured bound: below the 700-word floor the prompt sets for
    // moderate/strong. Padding judgment beyond that needs a human read.
  ...
  console.log(`  (explorer-facing words: ${words})`);
  return fails;
}
...
// Dump non-crisis outputs for human review (voice quality needs a human read).
const { writeFileSync } = await import('fs');
writeFileSync('/tmp/effmap-v16-pressure-test.json', JSON.stringify(results, null, 2));
console.log('full outputs: /tmp/effmap-v16-pressure-test.json');
